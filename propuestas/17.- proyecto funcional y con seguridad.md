...
### 💎 PARCHE DE SEGURIDAD ABSOLUTA (La Válvula Inquebrantable)

**Von Neumann:** Vamos a aplicar una actualización quirúrgica. Eliminaremos ese bloque "suicida". Al hacerlo, tanto el modo `PARALLEL_INITIAL` como el modo `RANDOM` estarán **sometidos al mismo embudo estricto de seguridad**.

Si usted lanza 10,000 tareas en `PARALLEL_INITIAL` y le pone un límite de `50`, el Orquestador lanzará las primeras 50 al mismo tiempo, y luego usará una "ventana deslizante" para ir inyectando las demás conforme se liberen espacios. **El servidor jamás volverá a colapsar.**

Por favor, aplique esta actualización a su motor:

```sql
BEGIN;

-- ============================================================================
-- PROCEDIMIENTO: bg.bg_job_orchestrator (PARCHE DE VÁLVULA INQUEBRANTABLE)
-- CORRECCIÓN: Se elimina el override suicida de PARALLEL_INITIAL.
-- RESULTADO: max_parallel_processes ahora es una regla de hardware absoluta.
-- ============================================================================
CREATE OR REPLACE PROCEDURE bg.bg_job_orchestrator(p_run_id INT) AS $$
DECLARE
    v_mode bg.execution_mode; v_timeout INT; v_max_retries INT; v_max_parallel INT;
...
BEGIN
    ...
    -- Leemos la válvula de seguridad configurada por el usuario
    SELECT dj.mode, dj.timeout_seconds, dj.max_retries, dj.max_parallel_processes 
    INTO v_mode, v_timeout, v_max_retries, v_max_parallel
    FROM bg.run_jobs rj JOIN bg.def_jobs dj ON rj.job_id = dj.job_id WHERE rj.run_id = p_run_id;

    -- ❌ SE ELIMINÓ EL OVERRIDE DE PARALLEL_INITIAL PARA GARANTIZAR SEGURIDAD HARDWARE ❌
    ...
    -- LÓGICA 2: CONCURRENTE (Respeta la válvula max_parallel en TODOS los casos)
    ELSE
        WHILE EXISTS (SELECT 1 FROM bg.run_tasks WHERE run_id = p_run_id AND status IN ('PENDING', 'RUNNING')) LOOP
            COMMIT; 
            ...
            -- EMBÚDO SEGURO: Jamás lanzará más Workers de los permitidos por el usuario
            WHILE v_active_slots < v_max_parallel LOOP
                v_run_task_id := NULL; 
                SELECT run_task_id INTO v_run_task_id FROM bg.run_tasks WHERE run_id = p_run_id AND status = 'PENDING' ORDER BY execution_order ASC LIMIT 1;
                EXIT WHEN v_run_task_id IS NULL; 
...
### 💎 PARCHE DE SEPARACIÓN DE MODOS (El Motor Realmente Aleatorio)

Hemos modificado el Orquestador (`bg_job_orchestrator`). Hemos reemplazado la línea genérica por un condicional inteligente que le da vida real al modo `RANDOM`.

### ❌ El Código Anterior (El Error)

Antes del parche, el Padre siempre sacaba las tareas formaditas en fila india (1, 2, 3...), sin importar si el usuario había elegido `RANDOM` o `PARALLEL_INITIAL`. Era un comportamiento estático.
...
-- ============================================================================
-- PROCEDIMIENTO: bg.bg_job_orchestrator (PARCHE DE IDENTIDAD DE MODOS)
-- CORRECCIÓN: Separación lógica del consumo de cola (Ordered vs Random).
-- ============================================================================
CREATE OR REPLACE PROCEDURE bg.bg_job_orchestrator(p_run_id INT) AS $$
DECLARE
    v_mode bg.execution_mode; v_timeout INT; v_max_retries INT; v_max_parallel INT;
    ...
    ELSE
        WHILE EXISTS (SELECT 1 FROM bg.run_tasks WHERE run_id = p_run_id AND status IN ('PENDING', 'RUNNING')) LOOP
            ...
            WHILE v_active_slots < v_max_parallel LOOP
                ...
                -- 🚀 MAGIA DEVSECOPS: Separación real de comportamientos
                IF v_mode = 'RANDOM' THEN
                    -- Extrae al azar para evadir contención de índices en disco
                    SELECT run_task_id INTO v_run_task_id FROM bg.run_tasks WHERE run_id = p_run_id AND status = 'PENDING' ORDER BY RANDOM() LIMIT 1;
                ELSE
                    -- PARALLEL_INITIAL: Extrae de forma ordenada y predecible
                    SELECT run_task_id INTO v_run_task_id FROM bg.run_tasks WHERE run_id = p_run_id AND status = 'PENDING' ORDER BY execution_order ASC LIMIT 1;
                END IF;
